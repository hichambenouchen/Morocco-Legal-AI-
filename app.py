...
# ---------------------------------------------------------
# 3. دالتا إنشــاء ملفات DOCX و PDF
# ---------------------------------------------------------
def generate_docx(messages):
  doc = docx.Document()

  # ضبط اتجاه الفقرة لليمن (RTL) في Word
  def set_rtl(p):
...
def generate_pdf(messages):
  html_content = """
    ...
    <head>
        ...
        <style>
            ...
            .user-msg .role-title { color: #2b6cb0; }
            .assistant-msg .role-title { color: #276749; }
            .content {
                font-size: 10pt;
    ...
    <body>
        <div class="header">
            <h1>⚖️ تقرير استشارة قانونية</h1>
            <p style="font-size: 10pt; color: #4a5568;">مدونة الشغل المغربية (القانون رقم 65.99)</p>
        </div>
    """
    ...
    if msg['role'] == 'user':
      html_content += f"""
            <div class="message-box user-msg">
                <div class="role-title">👤 السؤال / الاستفسار:</div>
      ...
      html_content += f"""
            <div class="message-box assistant-msg">
                <div class="role-title">⚖️ الرد والمطابقة القانونية:</div>
                <div class="content">{msg['content']}</div>
            </div>
            """
  ...
  html_content += """
        <div class="footer">
            تم استخراج هذا التقرير آلياً بواسطة نظام المستشار القانوني الذكي - هذه الاستشارة ذات طابع أكاديمي واسترشادي.
        </div>
    </body>
    </html>
    """
...
with st.expander('⚙️ خيارات وأدوات الاستشارة وتحميل التقارير'):
  ...
  col_reset, col_txt, col_docx, col_pdf = st.columns(4)
